...
<div role="tabpanel" class="tab-pane" id="user" style="display:block;">
	<div class="check-div form-inline row">
				<div class="col-xs-1">
                    <a href="{:url('create')}"><button class="btn btn-yellow btn-xs" data-toggle="modal" data-target="#addUser" id="create">添加{$modelName}</button></a>
		</div>
        <div class="col-xs-11">
...
<?php foreach($list_cate as $row_cate){?>
    <option value="{$row_cate->id}"  {eq name="Think.get.cate_id"  value="$row_cate->id" }selected{/eq} >{$row_cate->name}</option>
<?php }?>
	...
	<div class="data-div">
		...
		<div class="tablebody">
			...
			<div class="row cont_nowrap">
                ...
                <div class="col-xs-1 " title="{$row_->vid}">
                    {$row_->vid}
                </div>
